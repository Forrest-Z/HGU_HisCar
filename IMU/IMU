...
//	unsigned char id[Length] = "id\n";
	...
	pkt_init(pkt_send); //initialize packet.

    /****Request to read objects***/
...
//	}
    ...
    char *tr;
    char buf[] = "37.111 55.222 -444.4 55.111 44.555 66.555 11.111 22.222 33.333";
    char blank[] = " ";
    int n =0; // n is the number of byte read.
    ...
    printf("contents : %s \n", buf); //show what was read from fd.
    
    char *x,*y,*z;
    
    tr = strtok(buf,blank);
    for(int i = 0; tr!=NULL;i++)
    {
        ...
        tr = strtok(NULL,blank);
        
    }


    
    //object_write_request(pkt_send,)
